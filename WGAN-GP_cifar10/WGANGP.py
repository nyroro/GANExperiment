...
class WGANGP():
    def __init__(self):
        ...
        import pickle
        import os
        if os.path.exists('noise_sample.pkl'):
            with open('noise_sample.pkl', 'rb') as f:
                self.noise_sample = pickle.load(f)
        else:
            self.noise_sample = np.random.normal(0, 1, (r * c, self.latent_dim))
            with open('noise_sample.pkl', 'wb') as f:
                pickle.dump(self.noise_sample, f)
        ...
        self.critic.load_weights('wgan_D_1.h5')
    ...
    def sample_images(self, epoch):
        gen_imgs = self.generator.predict(self.noise_sample)
