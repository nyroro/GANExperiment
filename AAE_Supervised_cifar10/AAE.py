...
class AdversarialAutoencoder():
    def __init__(self):
        ...
        reconstructed_img = self.decoder([encoded_repr, labels])
    ...
    def build_decoder(self):
        ...
        model = Sequential()
        model.add(Dense(4*4*4*128, input_dim=self.latent_dim+self.nclasses))
        ...
        labels = Input(shape=(self.nclasses,))
        inputs = Concatenate()([z, labels])
        img = model(inputs)

        return Model([z, labels], img)
    ...
    def sample_images(self, epoch):
        ...
        labels = np.array([t%self.nclasses for t in range(r*c)])
        labels = keras.utils.to_categorical(labels, self.nclasses)

        gen_imgs = self.decoder.predict([z, labels])
